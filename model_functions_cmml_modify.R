...
computeAC_performancev2 <- function(risk, Schema_res, Item_EI, Outputs_cho, ThisPhase, schemainfo){
  match_count <- sum(Schema_res == unique(Schema_res)[1])
...
## Model Main Function
simulation <- function(Param.df, type = 'L', exp_type = 'painting', save=F, savepath="",
  ...
  for (Subject in 1:subjectnum) {
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      while (decisiontime <= 2){ ## i.e. 2 phase to choose item
        ...
        ## **Decisiontime 2：依据 AC 计算 shift_prob 并选择 Schema**
        if (decisiontime == 2) {
          if (!is.na(last_AC)) {  # 确保有上一回合的 AC 记录
            shift_prob <- cal_shift_prob(last_AC)  # 计算 Shift Probability
            
            if (runif(1) < shift_prob) {
              #shifting
              ChoSchema = Schema_EI[!Schema_EI$schemaID %in% Thischosen$chosen,]
            } else {
              #staying
        ...
        # **更新 last_AC**（记录 Decision 1 的 Schema 是否成功）
        if (decisiontime == 1) {
          last_AC <- Outputs_cho$AC[ThisPhase]  # 记录当前回合的 AC 值
        }
