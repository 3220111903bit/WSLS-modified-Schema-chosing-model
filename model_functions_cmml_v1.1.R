...
computeAC_performance <- function(risk, Schema_res, Item_EI, Outputs_cho, ThisPhase, schemainfo){
  if (risk == 'high'){ #  high risk
    ...
    if (length(unique(Schema_res)) == 1){
      ...
      AC = 0.5
      right_schema =  as.data.frame(so(table(Schema_res), decreasing=TRUE))[1,1] # most frequent schema chosen
...
simulation <- function(Param.df, type, exp_type, save=F, savepath="",
  ...
  for (Subject in 1:subjectnum) {
    ...
    ##### except for the schema selection pa, other pas are the same as what we discussed
    #### create a data frame to store the outputs ####
    
    # 是reaction time 指你花了多少时间反应 比如1就是选出第一个item的时间
    ...
    Outputs_cho = data.frame(Subject = c(rep(Subject, max_Round*2)), Round = c(rep(1:(max_Round*2),each=2)),Phase = c(1:(max_Round*2)),
                             Schema = c(rep(0,max_Round*2)),Schema_ = c(rep(0,max_Round*2)),
                             ...
                             Cho_4 = c(rep(0,max_Round*2)),_1 = c(rep(0,max_Round*2)), _2 = c(rep(0,max_Round*2)), 
                             _3 = c(rep(0,max_Round*2)), _4 = c(rep(0,max_Round*2)),OB_1 = c(rep(0,max_Round*2)), 
    ...
    ##### create the experiment for fuher simulation ######
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      # aificial clock signal if we only simulate afterbreak
      ...
      if (break_happens == 0 & clock >= 1500){
        ...
        # if we want to simulate after: we impo the last set of confidence for each Subject
        if(sim.mode=="after"){
          ...
          if(after.read.best.only){ #kan bu dong, you dian chun
            ...
            if(!(random.subj  %in% ref.schemainfo$Subject)){
              ...
              print('before pa of estimation nonexistent, continued')
              error <- 1
              break
            }
      ...
      while (decisiontime <= 2){ ## i.e. 2 phase to choose item
        if (clock >= 5500 | Sys.time() - sta.time > 240){
        ...
        #### let's sta the evidence integration ####
        schema_decision = 0
        # Finish: 已选择的个数
        Finish = 0
        ...
        while (Finish <= 4){ ## i.e. we sta to choose 4 item
          ...
          while (shift == 0) { ### i.e., when the attention does not shift
            ...
            if (Item_EI$evidence[Item_EI$decision == 0 & 
              ...
              if (schema_decision != 0 & 
                ...
                Outputs_cho[ThisPhase, paste0("Cho_", Finish)] = Item_EI$ID[Item_EI$evidence >= Item_EI$thres &
                                                                              ...
                                                                              Item_EI$ID == attention & 
                                                                              Item_EI$status == 1]
                Outputs_cho[ThisPhase, paste0("_", Finish)] = CTime
            ...
            if (schema_decision == 0){
              ...
              if (any(ChoSchema$evidence >= thres_schema)){  ## i.e. schema identified
                ...
                Outputs_cho$Schema_[ThisPhase] = CTime
    ...
    if (error != 1){
      ...
      Outputs_cho = Outputs_cho[Outputs_cho$_4!=0, ]
...
## Impoant: Comment out before impoing in other script!
...
res <- simulation(Param.df, "L", "painting", save = T,
                  savepath = "D:/Users/Xin/Downloads/CMML_mini1/sim_try1", sim.mode="before",
