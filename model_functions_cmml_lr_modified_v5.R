...
#changing the confidence mechanism of the model
#restricting the exploration phase Cons < 0.3 / decay mechanism for un-learned schema
...
simulation <- function(Param.df, type, exp_type, save=F, savepath="",
  ...
  for (Subject in 1:subjectnum) {
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      # 使用上一个 round 的两个 phase 中选择的 schema 判断是否更新或衰减
      if (ThisRound > 1) {
        prev_phases <- Outputs_cho %>% filter(Round == (ThisRound - 1))
        chosen_schema_ids <- unique(prev_phases$Schema)
      } else {
        chosen_schema_ids <- c()  # 第一轮默认无选择
      }
      
      for (i in 1:nrow(schemainfo)) {
        sid <- schemainfo$schemaID[i]
        prior_con <- Con_PriorExp[i]
        after_con <- Con_afterExp[i]
        chosen_flag <- sid %in% chosen_schema_ids
        
        if (after_con <= 0.4) {
          schemainfo$conN[i] <- schemainfo$conN[i] + Beta_N * (after_con - prior_con)
        } else {
          if (chosen_flag) {
            schemainfo$conN[i] <- schemainfo$conN[i] + Beta_N * (after_con - prior_con)
          } else {
            schemainfo$conN[i] <- schemainfo$conN[i] * 0.4
          }
        }
      }
...
#        }
        ...
        if (all(schema_pattern == c(4))) { # 1111
          ...
          schemainfo$conN[idx] <- schemainfo$conN[idx] + lr * delta
          ...
          schemainfo$conN[idx_dom] <- schemainfo$conN[idx_dom] + lr * delta_dom
          
          idx_int <- schemainfo$schemaID == interference_schema
          delta_int <- abs(AC - Schema_EI$Scon[Schema_EI$schemaID == interference_schema])
        
          schemainfo$conN[idx_int] <- schemainfo$conN[idx_int] - lr_neg * delta_int
          ...
          idx_dom <- schemainfo$schemaID == dominant_schema
          delta_dom <- AC - Schema_EI$Scon[Schema_EI$schemaID == dominant_schema]
          schemainfo$conN[idx_dom] <- schemainfo$conN[idx_dom] + lr * delta_dom  # 增幅较小
          ...
          for (sid in schemas) {
            idx <- schemainfo$schemaID == sid
            delta <- abs(AC - Schema_EI$Scon[Schema_EI$schemaID == sid])
            schemainfo$conN[idx] <- schemainfo$conN[idx] - lr_neg * delta
          }
