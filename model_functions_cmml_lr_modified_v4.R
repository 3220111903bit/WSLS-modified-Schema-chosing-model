...
#allowing the scheme to stay non-shifting
#w change from 0.3 to 0.6 / beta_learning rate (v1 0.3/0.25 --> v2 0.4/0.3 --> v3 0.7/0.4)
#try4: 0.4/0.3, removing payoff try5:0.8/0.5 removing payoff
...
adjust_lr_asym <- function(beta, RT, Round, AC,
                           RT_ref, Round_ref = 3,
                           k1 = 0.5, k2 = 1) {
  # 1. 计算 f(RT, Round)
  exponent <- -k1 * ((RT_ref - RT) / RT_ref) - 
    k2 * ((Round - Round_ref) / Round_ref)
  f_val <- exp(exponent)
...
simulation <- function(Param.df, type, exp_type, save=F, savepath="",
                       ...
                       scale.confi.init, beta_pos1 = 0.8, beta_pos2 = 0.5){
  ## df initialization
  ...
  for (Subject in 1:subjectnum) {
    ...
    if (!exists("RT_history")) {
      RT_history <- c()  # 用于存储每轮实验的RT
    }
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      if (break_happens == 0 & clock >= 1500){
        ...
        explorationtime = 300 ## exploration time is 5 min right after break
      ...
      while (decisiontime <= 2){ ## i.e. 2 phase to choose item
        ...
        Schema_EI$DM = mapply(Bonus,Schema_EI$weightCon)
...
#        }
        ...
        RT <- Outputs_cho$Schema_RT[ThisPhase]
        RT_history <- c(RT_history, RT)
        RT_ref <- mean(RT_history)
        
        selected_schemas <- Item_EI$Schema[Item_EI$ID %in% Outputs_cho[ThisPhase, c("Cho_1","Cho_2","Cho_3","Cho_4")]]
        schema_count <- table(selected_schemas)
        schema_pattern <- sort(as.numeric(schema_count), decreasing = TRUE)
        
        AC <- Outputs_cho$AC[ThisPhase]
        Round_val <- ThisRound
        ...
        beta_use <- if (AC == 1) {
          ...
          Param.df$Beta_N[Subject]
        }
        
        lr <- adjust_lr_asym(beta = beta_use, RT = RT,RT_ref = RT_ref, Round = Round_val, AC = AC)
        lr_neg <- adjust_lr_asym(beta = Beta_N, RT = RT, RT_ref = RT_ref, Round = Round_val, AC = AC)
        
        # 根据模式采取策略
        if (!exists("Outputs_delta")) {
          Outputs_delta <- list(`0` = c(), `0.5` = c(), `1` = c())
        }
        
        # 根据模式采取策略并同时保存delta
        if (all(schema_pattern == c(4))) { # 1111
          dominant_schema <- as.numeric(names(schema_count)[1])
          idx <- schemainfo$schemaID == dominant_schema
          delta <- AC - Schema_EI$Scon[Schema_EI$schemaID == dominant_schema]
          
          # 记录实际更新量
          real_delta <- lr * delta
          Outputs_delta[[as.character(AC)]] <- c(Outputs_delta[[as.character(AC)]], real_delta)
          
          # 更新confidence
          schemainfo$conN[idx] <- schemainfo$conN[idx] + real_delta
          
        } else if (all(schema_pattern == c(3,1))) { # 1110
          dominant_schema <- as.numeric(names(schema_count)[schema_count == 3])
          interference_schema <- as.numeric(names(schema_count)[schema_count == 1])
          
          # 主导schema
          idx_dom <- schemainfo$schemaID == dominant_schema
          delta_dom <- AC - Schema_EI$Scon[Schema_EI$schemaID == dominant_schema]
          real_delta_dom <- lr * delta_dom
          Outputs_delta[[as.character(AC)]] <- c(Outputs_delta[[as.character(AC)]], real_delta_dom)
          schemainfo$conN[idx_dom] <- schemainfo$conN[idx_dom] + real_delta_dom
          
          # 干扰schema
          idx_int <- schemainfo$schemaID == interference_schema
          delta_int <- abs(AC - Schema_EI$Scon[Schema_EI$schemaID == interference_schema])
          real_delta_int <- -lr_neg * delta_int
          Outputs_delta[[as.character(AC)]] <- c(Outputs_delta[[as.character(AC)]], real_delta_int)
          schemainfo$conN[idx_int] <- schemainfo$conN[idx_int] + real_delta_int
          
        } else if (all(schema_pattern == c(2,2))) { # 1100
          schemas <- as.numeric(names(schema_count))
          for (sid in schemas) {
            idx <- schemainfo$schemaID == sid
            delta <- abs(AC - Schema_EI$Scon[Schema_EI$schemaID == sid])
            real_delta <- lr * delta
            Outputs_delta[[as.character(AC)]] <- c(Outputs_delta[[as.character(AC)]], real_delta)
            schemainfo$conN[idx] <- schemainfo$conN[idx] + real_delta
          }
          
        } else if (all(schema_pattern == c(2,1,1))) { # 1102
          dominant_schema <- as.numeric(names(schema_count)[schema_count == 2])
          interference_schemas <- as.numeric(names(schema_count)[schema_count == 1])
          ...
          # 干扰schemas
          for (sid in interference_schemas) {
            idx_int <- schemainfo$schemaID == sid
            delta_int <- abs(AC - Schema_EI$Scon[Schema_EI$schemaID == sid])
            real_delta_int <- -lr_neg * delta_int
            Outputs_delta[[as.character(AC)]] <- c(Outputs_delta[[as.character(AC)]], real_delta_int)
            schemainfo$conN[idx_int] <- schemainfo$conN[idx_int] + real_delta_int
          }
          ...
          for (sid in schemas) {
            idx <- schemainfo$schemaID == sid
            delta <- abs(AC - Schema_EI$Scon[Schema_EI$schemaID == sid])
            real_delta <- -lr_neg * delta
            Outputs_delta[[as.character(AC)]] <- c(Outputs_delta[[as.character(AC)]], real_delta)
            schemainfo$conN[idx] <- schemainfo$conN[idx] + real_delta
          }  
            schemainfo$conVar[idx] <- schemainfo$conVar[idx] + 
              Beta_Var * (abs(delta) - schemainfo$conVar[idx])
  ...
  if(
    #sim.mode == "before" | 
    save.confi == T){
    Outputs_delta_df <- data.frame(
      AC = rep(names(Outputs_delta), lengths(Outputs_delta)),
      delta = unlist(Outputs_delta, use.names = FALSE)
    )
    write_csv(Outputs_delta_df, file.path(simupath,'outputs_delta.csv'))
