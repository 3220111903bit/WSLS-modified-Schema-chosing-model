  ...
  for (Subject in 1:subjectnum) {
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      while (decisiontime <= 2){ ## i.e. 2 phase to choose item
        ...
        schema_ids <- unique(Schema_res)
        for (sid in schema_ids) {
          idx <- schemainfo$schemaID == sid
          this_scon <- Schema_EI$Scon[Schema_EI$schemaID == sid]
          delta <- Outputs_cho$AC[ThisPhase] - this_scon
          ...
          schemainfo$conN[idx] <- schemainfo$conN[idx] + beta_use * delta
